...
**票 01、02、03、06、07 皆已完成**（`Status: resolved`，檢查清單保留打勾）。本資料夾只剩票 04 與 05 開著。
...
- **票 04、05** —— 仍為 `needs-triage`，票裡的待決定問題尚未討論。這是本資料夾唯一還開著的兩張票。
- **計時範圍** —— `spec.md`「實作後補記的行為變化」記載了兩項超出原先預期的行為變化。還原耗時不再涵蓋安全快照一項已與使用者確認維持現狀（理由：快照本身已有自己的紀錄與耗時，舊算法等於把同一段時間計入兩筆紀錄）。
- **其餘候選** —— 同一次架構檢視的候選 1、2、4、5、6 尚未排程，見 `spec.md` 的 Out of Scope 末段。建議順序候選 2 → 1 → 6。

（票 06 與票 07 皆已 `resolved`。）

## 3. 怎麼跑

```
dotnet build --no-incremental   # 必須 0 警告 0 錯誤
dotnet test                     # 交接時 192 個全綠；票 01–03、06、07 完成後為 212（Core 114、CLI 54、WPF 44）
```
...
票 06 的「剩餘工作」（型別重整）刻意排在 01–03 之後，已於其後完成，做法與原規劃不同，原因寫在票裡。
